...
数据存在当前目录的 `.decision-debt/ledger.json`，也可用 `--ledger 路径` 或环境变量 `DECISION_LEDGER` 指定。<!-- dd:ignore: 本地运行时产物，不随仓库提交 -->
...
├── tests/test_debt.py             # 验收测试套件
├── examples/build_examples.py     # 用真实工具复现样例的脚本
├── examples/sample-ledger.json    # 样例台账
├── examples/sample-report.txt     # 样例报告
├── examples/sample-export.md      # 样例决策日志
